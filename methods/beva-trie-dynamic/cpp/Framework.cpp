...
namespace beva_trie_dynamic {
    ...
    void Framework::index() {
        ...
        switch (this->dataset) {
            ...
            case C::JUSBRASIL:
                ...
                break;
            default:
                datasetFile = "/home/berg/workspace/mestrado/beva-dynamic/test.txt";
                queryFile = "/home/berg/workspace/mestrado/beva-dynamic/q.txt";
                break;
        }
    ...
    vector<char *> Framework::processFullQuery(string &query, int queryId) {
        ...
        vector<char *> results = this->output(currentActiveNodes, query);

#ifdef BEVA_IS_COLLECT_TIME_H
        experiment->endSimpleQueryFetchingTime(results.size());

        bool relevantReturned = false;
        if (queryId != -1 && config["has_relevant_queries"] == "1") {
    ...
    void Framework::process(string query, int prefixQueryLength, int currentCountQuery,
        ...
        if (std::find(prefixQuerySizeToFetching.begin(), prefixQuerySizeToFetching.end(), prefixQueryLength) !=
            ...
            experiment->initQueryFetchingTime();
            vector<char *> results = output(currentActiveNodes, query.substr(0, prefixQueryLength));
    ...
    void transverseTrie(Node *node, vector<char *> &outputs, int &limit) {
        if (node->getRecordId() != -1) {
            outputs.push_back(records[node->getRecordId()].c_str());
        }

        for (auto & child : node->children) {
            transverseTrie(&child, outputs, limit);
...
//            }
    ...
    vector<char *> Framework::output(vector<ActiveNode> &currentActiveNodes, const string &query) {
        vector<char *> outputs;
        string tmp;
        int limit = 1000;

        for (ActiveNode activeNode : currentActiveNodes) {
            transverseTrie(activeNode.node, outputs, limit);
        }
